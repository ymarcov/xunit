...
namespace Xunit.Runner.v3
{
	...
	public class TcpExecutionEngine : TcpEngine
	{
		...
		Task Execute(string operationID)
		{
			return Task.Run(() =>
			{
				...
				try
				{
					...
					var filteredTestCases = discoverySink.TestCases.Where(projectAssembly.Configuration.Filters.Filter).ToList();
		...
		class EngineDiscoverySink : _IMessageSink
		{
			...
			readonly Func<_TestCaseDiscovered, bool> filter;
			readonly string operationID;

			public EngineDiscoverySink(
				TcpExecutionEngine engine,
				string operationID,
				Func<_TestCaseDiscovered, bool> filter)
			{
				this.engine = engine;
